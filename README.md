...
│   └── version.cfg              # Dev defaults; overwritten by CI on every deploy
│
├── scenes/
│   ├── characters/
│   ├── enemies/
...
#### Step 3a — Write Version

Overwrites `$WORKSPACE/resources/version.cfg` with build-specific values before the
import/export steps so they get baked into the exported `.pck`:
...
[channel]
name="staging"   # "release" on the main branch

[build]
commit="<7-char git sha>"
id="<BUILD_ID from Set Build ID>"
date="<UTC ISO-8601 timestamp>"
```

`scripts/version_label.gd` reads this file at runtime and renders it as e.g.
`v0.1.0-staging-a1b2c3d`. The `major`/`minor`/`patch` values are currently hardcoded in
this step — bump them here when cutting a release. The committed
`resources/version.cfg` is only a fallback for local editor runs, where it always
displays `v0.1.0-dev-local`.

> This step writes a plain text file via `printf`, not a heredoc. A `cat <<EOF` heredoc
> here will break the workflow: YAML literal block scalars (`run: |`) require every line
> — including the heredoc's closing delimiter — to share the step's indentation, but
> `<<EOF` (unlike `<<-EOF`) requires that delimiter at column 0. The two requirements
> are incompatible, so the heredoc never terminates and swallows the rest of the file.
...
| `include_filter` | `resources/version.cfg` | `.cfg` files aren't bundled by `export_filter=all_resources`; this re-includes the version file so `version_label.gd` can read it at runtime |
...
**`scripts/version_label.gd`**

Attach to any `Label`. Reads `resources/version.cfg` via `ConfigFile` in `_ready()` and
renders `v{major}.{minor}.{patch}`, then appends `-{channel}` unless the channel is
`release`, then always appends `-{commit}` -- e.g. `v0.1.0-staging-a1b2c3d` on staging
or `v0.1.0-a1b2c3d` on a release build. Falls back to `v?.?.?` if the file is missing or
fails to load. CI overwrites `version.cfg` with real build metadata before export (see
Step 3a in the CI/CD Pipeline section) -- locally in the editor it always reads the
committed dev defaults (`v0.1.0-dev-local`).
